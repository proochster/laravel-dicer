<template>
    <div>
        ...
        >
            <span class="text-capitalize text-info">{{ room_name }}</span>
            <button
                class="btn btn-sm"
...
export default {
    ...
    created() {
         ...
         */
        axios.get(`/api/room/${this.room_hash}/messages`).then(response => {
            // Limit the number of messgaes
            this.messages = response.data.slice(
                Math.max(response.data.length - 50, 1)
            );
        });
    },
    ...
    mounted() {
         ...
         */
        window.Echo.channel(`room-channel.${this.room_hash}`).listen(
            "NewMessage",
            e => {
    ...
    methods: {
        ...
        setUserData(data) {
            ...
            this.userID = data.id;
            document.cookie = `diceroom_user_token=${
                data.hash
            }; expires=${new Date(
                new Date().getTime() + 1000 * 60 * 60 * 24 * 365
            ).toGMTString()}; path=/;`;
            document.cookie = `diceroom_user_name=${
                data.name
            }; expires=${new Date(
                new Date().getTime() + 1000 * 60 * 60 * 24 * 365
            ).toGMTString()}; path=/;`;
            document.cookie = `diceroom_user_ID=${data.id}; expires=${new Date(
                new Date().getTime() + 1000 * 60 * 60 * 24 * 365
            ).toGMTString()}; path=/;`;
        },
